...
# 🔹 SEO-ключевые слова
SEO_KEYWORDS = [
    "сонник", "толкователь снов", "осознанные сновидения", "к чему снится", "приснилось что",
    "сон", "значение снов", "сон во сне", "видеть во сне", "сонник онлайн",
    "сонник нейросеть", "бессонница как уснуть", "кошмар", "сонник толкование", "сонник толкование бесплатно"
]
...
ADVERTISEMENTS = [
    ...
    "📲 Спросите Нейросеть Somnia AI про ваш сон в приложении → RuStore https://www.rustore.ru/catalog/app/com.somniaai.flutterapp"
]
...
def generate_post(topic, platform, length, style):
    try:
        # SEO-ключи 
        selected_keywords = random.sample(SEO_KEYWORDS, k=2) if platform in ["vk", "blog"] else []
        additional_keywords = generate_seo_keywords(topic) if platform in ["vk", "blog"] else []
        all_keywords = selected_keywords + additional_keywords[:2]  # Ограничиваем до 4 ключей


        # 🔹 📌 Выводим SEO-ключи в консоль для проверки
        if platform in ["vk", "blog"]:
            print(f"📌 SEO-ключи: {all_keywords}")        

        prompt = f"""
        Вы известный профессиональный психолог, пишущий захватывающие посты для {platform.upper()} на тему "{topic}" с ярким заголовком.
        Рассказ интересный и вдохновляющий с философским и глубоким смыслом в стиле "{style}".
        Введение должно раскрывать тему, не используя фразу 'Сны — это'. Используйте ключевые слова {' и '.join(all_keywords)} 2-3 раза в тексте.
        Формат текста: {"короткий и емкий, в 2-3 абзаца" if length == "short" else "лонгрид, в 4-5 абзацев с заголовками и примерами"}.
