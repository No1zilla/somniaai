...
def generate_post(topic, platform, length, style):
    try:
        ...
        prompt = f"""
        Вы психоналитик снов, пишущий пост для {platform.upper()} на тему "{topic}".
        Текст интересный и вдохновляющий в стиле "{style}".
        Используйте ключевые слова {' и '.join(all_keywords)} 2-3 раза в тексте.
        Формат текста: {"структурированный, короткий и емкий с примерами, в 1-2 абзаца" if length == "short" else "лонгрид, в 4-5 абзацев с заголовками и примерами"}.
        Избегайте повторений. В конце добавьте 1-2 вопроса для обсуждения, используйте среднее количество эмодзи.
        """
